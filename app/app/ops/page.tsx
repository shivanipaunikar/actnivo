...
const tabs = [
  ["all", "All"], ["critical", "Critical"], ["needs_approval", "Needs Approval"], ["running", "Running"],
...
export default async function OpsInboxPage({ searchParams }: { searchParams: Promise<{ status?: string; scan?: string; error?: string }> }) {
  ...
  return <div className="product-page ops-page">
    ...
    {!all.length ? <section className="inventory-empty"><span>◎</span><small>NO OPERATIONAL ISSUES</small><h2>{"There’s nothing to prioritize yet."}</h2><p>Import inventory and at least 28 days of sales history, then run deterministic detection.</p><Link className="saas-primary" href="/app/integrations/import">Upload operations data</Link></section> : !items.length ? <section className="ops-empty-filter"><p>No issues match this view.</p></section> : <section className="ops-list">{items.map(({ issue, sku, location, recommendation, sourceLocation }, index) => <Link href={`/app/ops/issues/${issue.id}`} key={issue.id} className="ops-issue-card">
      <span className={`ops-severity ${issue.severity}`}><i />{issue.severity}</span>
      <div className="ops-issue-main"><small>{String(index + 1).padStart(2, "0")} · {issue.type.replaceAll("_", " ")}</small><h2>{sku?.product_name ?? issue.title}</h2><p>{issue.summary}</p><div><span>{issue.channel ? titleCaseChannel(issue.channel) : "All channels"}</span><span>{location?.name ?? "All locations"}</span><span>{Number(issue.days_of_cover ?? 0).toFixed(1)} days remaining</span></div></div>
      <div className="ops-impact"><small>REVENUE AT RISK</small><strong>{currency.format(Number(issue.estimated_revenue_at_risk ?? 0))}</strong><span>{issue.estimated_shortage_units ?? 0} expected shortage units</span></div>
      <div className="ops-recommend"><small>RECOMMENDED ACTION</small>{recommendation ? <><strong>Move {recommendation.quantity} units</strong><span>{sourceLocation?.name ?? "Source"} → {location?.name ?? "Destination"}</span></> : <><strong>Replenishment required</strong><span>No safe source location found</span></>}</div>
      <b>→</b>
    </Link>)}</section>}
  </div>;
}
