...
function recommendationCopy(issue: any, recommendation: any, sourceLocation: any, location: any) {
  ...
  if (String(metadata.recommendation_type ?? "") === "CREATE_ORDER_RECOVERY_TASK") return { title: String(metadata.recommendation_title ?? "Create order recovery task"), detail: String(metadata.recommendation_detail ?? "Prepare an assisted operator task") };
  if (String(metadata.recommendation_type ?? "") === "CREATE_RETURN_RECOVERY_TASK") return { title: String(metadata.recommendation_title ?? "Create return/RTO recovery task"), detail: String(metadata.recommendation_detail ?? "Prepare an assisted reverse-logistics task") };
...
export default async function OpsInboxPage({ searchParams }: { searchParams: Promise<{ status?: string; scan?: string; error?: string }> }) {
  ...
  return <div className="product-page ops-page">
    ...
    {!all.length ? <section className="inventory-empty"><span>◎</span><small>NO OPERATIONAL ISSUES</small><h2>There’s nothing to prioritize yet.</h2><p>Connect inventory, sales, inbound supply, orders, and reverse-logistics data, then run deterministic detection.</p><Link className="saas-primary" href="/app/integrations/import">Upload operations data</Link></section> : !items.length ? <section className="ops-empty-filter"><p>No issues match this view.</p></section> : <section className="ops-list">{items.map(({ issue, sku, location, recommendation, sourceLocation }, index) => {
      const copy = recommendationCopy(issue, recommendation, sourceLocation, location);
      const metadata = issue.metadata as Record<string, unknown>;
      const isPo = String(issue.type).startsWith("PO_");
      const isOrder = ["ORDER_DELAYED", "ORDER_STUCK", "RTO_RISK"].includes(String(issue.type));
      const isReturn = ["RETURN_STUCK", "REFUND_DELAYED", "RETURN_RECEIPT_DELAYED"].includes(String(issue.type));
      return <Link href={`/app/ops/issues/${issue.id}`} key={issue.id} className="ops-issue-card">
        ...
        <div className="ops-issue-main"><small>{String(index + 1).padStart(2, "0")} · {String(issue.type).replaceAll("_", " ")}</small><h2>{isReturn ? String(metadata.external_return_id ?? issue.title) : isOrder ? String(metadata.external_order_id ?? issue.title) : sku?.product_name ?? issue.title}</h2><p>{issue.summary}</p><div><span>{issue.channel ? titleCaseChannel(issue.channel) : "All channels"}</span><span>{location?.name ?? "All locations"}</span><span>{isPo ? String(metadata.po_number ?? "Incoming supply") : isReturn ? `${String(metadata.kind ?? "RETURN")} · ${String(metadata.status ?? "open").replaceAll("_", " ")}` : isOrder ? `${String(metadata.payment_method ?? "ORDER")} · ${Number(metadata.delivery_attempts ?? 0)} attempts` : `${Number(issue.days_of_cover ?? 0).toFixed(1)} days remaining`}</span></div></div>
        <div className="ops-impact"><small>REVENUE AT RISK</small><strong>{currency.format(Number(issue.estimated_revenue_at_risk ?? 0))}</strong><span>{isReturn ? "refund + reverse-logistics exposure" : isOrder ? "order value exposed" : `${issue.estimated_shortage_units ?? 0} units at risk`}</span></div>
