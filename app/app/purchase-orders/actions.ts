...
export async function refreshPurchaseOrders() {
  ...
  let target = "/app/purchase-orders";
  try {
    ...
    target = `/app/purchase-orders?scan=${result.assessedLines}&risk=${result.risks}`;
  } catch (error) {
    target = `/app/purchase-orders?error=${encodeURIComponent(message(error))}`;
  }
  redirect(target);
}
...
export async function recordPurchaseOrderReceipt(formData: FormData) {
  ...
  let target = `/app/purchase-orders/${poId}`;
  try {
    assertCanManageInventory(context.role);
    ...
    revalidatePath("/app/purchase-orders");
    revalidatePath("/app/ops");
    target = `/app/purchase-orders/${poId}?received=1`;
  } catch (error) {
    target = `/app/purchase-orders/${poId}?error=${encodeURIComponent(message(error))}`;
  }
  redirect(target);
}
