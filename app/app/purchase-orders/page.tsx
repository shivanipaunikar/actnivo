...
export default async function PurchaseOrdersPage({ searchParams }: { searchParams: Promise<{ status?: string; risk?: string; scan?: string; error?: string }> }) {
  ...
  const statuses = [...new Set(workspace.purchaseOrders.map((po: any) => po.status))].sort();
  ...
  return <div className="product-page">
    ...
    <section className="product-card compact" style={{ marginTop: 16 }}><header><div><small>FILTERS</small><h2>Incoming supply</h2></div><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}><Link href="/app/purchase-orders">All</Link><Link href="/app/purchase-orders?risk=at-risk">At risk</Link>{statuses.map((status) => <Link key={status} href={`/app/purchase-orders?status=${status}`}>{String(status).replaceAll("_", " ")}</Link>)}</div></header></section>
