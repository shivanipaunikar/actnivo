...
export default async function PurchaseOrdersPage({ searchParams }: { searchParams: Promise<{ status?: string; risk?: string; scan?: string; error?: string }> }) {
  ...
  return <div className="product-page">
    <header className="product-page-header"><div><p>OPERATIONS / SUPPLY</p><h1>Purchase Orders</h1><span>Will incoming supply arrive in time to protect demand?</span></div><div style={{ display: "flex", gap: 10, flexWrap: "wrap", justifyContent: "flex-end" }}>{manageable && <Link href="/app/purchase-orders/import">Import PO file</Link>}{manageable && <form action={refreshPurchaseOrders}><button className="saas-primary" type="submit">Refresh supply intelligence</button></form>}</div></header>
