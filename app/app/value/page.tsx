...
export default async function ValuePage() {
  ...
  const [issues, actions, outcomes, recommendations, skus, locations, settings] = await Promise.all([
    ...
    db.from("organization_settings").select("monthly_software_cost").eq("organization_id", organization.id).maybeSingle(),
  ]);
  for (const result of [issues, actions, outcomes, recommendations, skus, locations, settings]) if (result.error) throw new Error(result.error.message);
  ...
  const monthlySoftwareCost = Number(settings.data?.monthly_software_cost ?? 0);
  const valueMultiple = monthlySoftwareCost > 0 ? s.actualValue / monthlySoftwareCost : null;
  ...
  return <div className="product-page value-page"><header className="product-page-header"><div><p>INTELLIGENCE</p><h1>Value Generated</h1><span>What Actnivo protected, grounded in actions and verified outcomes.</span></div><Link href="/app/analytics">Open Analytics →</Link></header>
    ...
    {!hasHistory ? <section className="inventory-empty" style={{ marginTop: 18 }}><span>↑</span><small>NO OPERATING HISTORY YET</small><h2>Value appears after the operating loop runs.</h2><p>Detect an issue, prepare and approve an action, then verify the outcome.</p><Link className="saas-primary" href="/app/ops">Open Ops Inbox</Link></section> : <>
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: 18, marginTop: 18 }}><article className="product-card"><header><div><small>WORKFLOW VALUE</small><h2>Verified value by workflow</h2></div></header><ValueList rows={value.byWorkflow} empty="No verified workflow value yet." /></article><article className="product-card"><header><div><small>SKU VALUE</small><h2>Top value-generating SKUs</h2></div></header><ValueList rows={value.bySku} empty="No verified SKU value yet." /></article></section>
      <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(320px,1fr))", gap: 18, marginTop: 18 }}><article className="product-card"><header><div><small>CHANNEL VALUE</small><h2>Verified value by channel</h2></div></header><ValueList rows={value.byChannel} empty="No verified channel value yet." /></article><article className="product-card"><header><div><small>LOCATION VALUE</small><h2>Verified value by location</h2></div></header><ValueList rows={value.byLocation} empty="No verified location value yet." /></article></section>
      <section className="product-card" style={{ marginTop: 18 }}><header><div><small>OPERATING IMPACT</small><h2>What Actnivo changed</h2></div></header><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 18, padding: "22px 24px 26px" }}><article><small>STOCKOUTS PREVENTED</small><strong style={{ display: "block", fontSize: 30, marginTop: 6 }}>{s.stockoutsPrevented}</strong><p>Successful stockout-risk verifications</p></article><article><small>INVENTORY REBALANCED</small><strong style={{ display: "block", fontSize: 30, marginTop: 6 }}>{s.inventoryRebalanced.toLocaleString("en-IN")} units</strong><p>Executed/verified transfer recommendations</p></article><article><small>AVG RESOLUTION TIME</small><strong style={{ display: "block", fontSize: 30, marginTop: 6 }}>{s.averageResolutionHours == null ? "—" : `${s.averageResolutionHours.toFixed(1)} hrs`}</strong><p>Action created to verified</p></article><article><small>AUTOPILOT VERIFIED VALUE</small><strong style={{ display: "block", fontSize: 30, marginTop: 6 }}>{money.format(s.autopilotActual)}</strong><p>Verified value from policy-prepared actions</p></article><article><small>MANUAL VERIFIED VALUE</small><strong style={{ display: "block", fontSize: 30, marginTop: 6 }}>{money.format(s.manualActual)}</strong><p>Verified value from operator-prepared actions</p></article></div></section>
      ...
      <section className="product-card" style={{ marginTop: 18 }}><header><div><small>ROI READINESS</small><h2>{valueMultiple == null ? "Configure software cost to compare value" : `${valueMultiple.toFixed(2)}× verified value / monthly cost`}</h2></div><Link href="/app/settings">Open Settings →</Link></header><div style={{ padding: "20px 24px 26px" }}>{valueMultiple == null ? <p>Monthly Actnivo cost has not been configured, so Actnivo will not fabricate an ROI number.</p> : <p>{money.format(s.actualValue)} verified value compared with {money.format(monthlySoftwareCost)} configured monthly software cost. This is a transparent value-to-cost multiple, not a period-matched accounting ROI unless the verified value shown covers the same month.</p>}</div></section>
    </>}
  </div>;
}
