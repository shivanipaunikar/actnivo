...
export default async function ImportJobPage({
  ...
  const suggested = { ...suggestColumnMapping(sourceType, headers), ...savedMapping };
  ...
  const isPurchaseOrders = sourceType === "purchase_orders";
  const completionHref = isPurchaseOrders ? "/app/purchase-orders" : sourceType === "sales" ? "/app/quick-commerce" : "/app/inventory";
  const completionLabel = isPurchaseOrders ? "View Purchase Orders" : sourceType === "sales" ? "Go to Quick Commerce" : "View inventory";
  const readyDescription = isPurchaseOrders
    ? "Purchase orders will be normalized into the source-agnostic PO model. Existing PO/line keys will be updated safely."
    : sourceType === "sales"
      ? "Validated sales rows will be added to daily sales history without replacing inventory snapshots."
      : "Inventory snapshots will be appended. Existing history will not be overwritten.";

  return (
    <div className="product-page import-job-page">
      <header className="product-page-header"><div><p>IMPORT / {sourceType.toUpperCase().replaceAll("_", " ")}</p><h1>{job.filename}</h1><span>{job.total_rows.toLocaleString("en-IN")} detected rows · uploaded {new Date(job.created_at).toLocaleString("en-IN")}</span></div><StatusBadge status={job.status} /></header>
