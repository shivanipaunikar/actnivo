...
export async function buildCopilotContext(supabase: SupabaseClient<Database>, organizationId: string) {
  ...
  const inventoryPosition = inventory.summaries.slice()
    ...
    .slice(0, 15).map((item) => ({
      sku: item.sku.master_sku, product: item.sku.product_name, available: item.available, reserved: item.reserved,
      inbound: item.inbound, sevenDaySales: item.sevenDaySales, daysOfCover: item.daysOfCover, status: item.status,
      locations: item.locations.map((location) => location.name), channels: item.channels, href: `/app/inventory/${item.sku.id}`,
    }));

  const poRisks = purchaseOrders.risks.slice(0, 12).map((risk) => ({
    poId: risk.poId, poNumber: risk.poNumber, poLineId: risk.poLineId, sku: risk.masterSku, product: risk.productName,
    supplier: risk.supplierName, destination: risk.destinationLocationName, expectedArrivalDate: risk.expectedArrivalDate,
    projectedStockoutAt: risk.projectedStockoutAt, gapDays: risk.gapDays, unitsAtRisk: risk.unitsAtRisk,
    revenueAtRisk: risk.revenueAtRisk, recommendationType: risk.recommendationType, transfer: risk.transfer ?? null,
  ...
  const channelHealth = quickCommerce.channelHealth.map((channel) => ({
    ...
    sevenDaySales: channel.recentSales, inventoryInStockPercent: channel.inventoryInStockPercent,
    availabilityConnected: channel.availabilityConnected,
  }));

  const pendingActions = actions.filter((item) => !["VERIFIED", "FAILED", "CANCELLED"].includes(item.action.status))
    .slice(0, 12).map((item) => ({
      id: item.action.id, type: item.action.type, status: item.action.status, executionMode: item.action.execution_mode,
      issueId: item.action.issue_id, estimatedValueProtected: Number(item.outcome?.estimated_value_protected ?? 0),
