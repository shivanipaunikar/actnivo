...
export type CopilotProposalType = "CREATE_TRANSFER_PLAN" | "CREATE_REPLENISHMENT_PLAN" | "EXPEDITE_PO" | "CREATE_ORDER_RECOVERY_TASK";
...
export async function prepareCopilotAction(
  supabase: SupabaseClient<Database>,
  organizationId: string,
  actorId: string,
  issueId: string,
  requestedType: CopilotProposalType,
) {
  ...
  if (existing.data) return { actionId: existing.data.id, created: false };

  let payload: Json;
  let estimatedValueProtected = Number(issue.estimated_revenue_at_risk ?? 0);

  if (requestedType === "CREATE_ORDER_RECOVERY_TASK") {
    if (!issue.order_id || !["ORDER_DELAYED", "ORDER_STUCK", "RTO_RISK"].includes(String(issue.type))) {
      throw new Error("This issue does not currently have an order-recovery recommendation.");
    }
    payload = {
      prepared_by: "AI_COPILOT",
    ...
    if (requestedType === "CREATE_TRANSFER_PLAN") {
      ...
      const calculation = asRecord(recommendation.calculation);
      estimatedValueProtected = Number(recommendation.estimated_revenue_protected ?? estimatedValueProtected);
      payload = {
        prepared_by: "AI_COPILOT",
        sku_id: sku.id,
        ...
        before_quantity: Number(calculation.destination_available ?? 0),
        expected_quantity: Number(calculation.destination_available ?? 0) + Number(recommendation.quantity),
      ...
      if (poResult.error) throw new Error("The purchase order for this issue is no longer available.");
      ...
      const destinationId = issue.location_id;
      if (!destinationId) throw new Error("This issue does not have a destination location for replenishment.");
      const destinationResult = await db.from("locations").select("*").eq("organization_id", organizationId).eq("id", destinationId).single();
      if (destinationResult.error) throw new Error("The destination location is no longer available.");
  ...
  const issueUpdate = await db.from("issues").update({ status: "needs_approval" })
    .eq("organization_id", organizationId).eq("id", issueId).neq("status", "ignored").neq("status", "resolved");
  ...
  const audit = await db.from("audit_events").insert({
    ...
    event_type: "copilot_action_prepared",
    before_state: null,
    after_state: { type: requestedType, issue_id: issueId, estimated_value_protected: estimatedValueProtected, payload } as Json,
  });
